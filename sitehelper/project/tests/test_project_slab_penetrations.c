...
static const char *path="project_slab_penetrations_test.txt";
...
static void test_validation(void)
{
    ...
    SlabPenetrationCollection *c=&s->definition.penetrations, saved=*c;
    SlabPenetrationCollection bad[]={{NULL,1,1},{saved.items,2,1},{saved.items,0,0}};
    for(size_t i=0;i<3;i++) {
        *c=bad[i]; check(&p,SITEHELPER_PROJECT_INVALID_SLAB_PENETRATION_COLLECTION); *c=saved;
    }
    ...
    for(size_t i=0;i<3;i++) {
        c->items[0].outline=bad_outline[i]; check(&p,SITEHELPER_PROJECT_INVALID_SLAB_PENETRATION_OUTLINE_COLLECTION);
    }
    ...
    c->items[0].outline.vertex_count=2; check(&p,SITEHELPER_PROJECT_INVALID_SLAB_PENETRATION_OUTLINE);
    ...
    const SiteHelperProjectValidationCode codes[]={SITEHELPER_PROJECT_INVALID_SLAB_PENETRATION_OUTLINE,
        SITEHELPER_PROJECT_SLAB_PENETRATION_OUTSIDE,SITEHELPER_PROJECT_SLAB_PENETRATION_OUTSIDE,
        SITEHELPER_PROJECT_SLAB_PENETRATION_OUTSIDE,SITEHELPER_PROJECT_SLAB_NUMERIC_OVERFLOW};
    for(size_t i=0;i<5;i++) { memcpy(outline.vertices,invalid[i],sizeof invalid[i]); check(&p,codes[i]); }
    memcpy(outline.vertices,hole,sizeof hole);
    assert(slab_add_penetration(s,second,4)==SLAB_SUCCESS); c=&s->definition.penetrations;
    const PlanPosition interactions[][4]={
        {{3000,2500},{4500,2500},{4500,3500},{3000,3500}},
        {{2500,2200},{3000,2200},{3000,2500},{2500,2500}},
        {{4000,3000},{4500,3000},{4500,3500},{4000,3500}}
    };
    for(size_t i=0;i<3;i++) {
        memcpy(c->items[1].outline.vertices,interactions[i],sizeof interactions[i]);
        check(&p,SITEHELPER_PROJECT_SLAB_PENETRATION_OVERLAP);
    }
    ...
    assert(!failed); /* Validation remains allocation-free. */
#endif
...
{
    fixture(p);
    assert(slab_add_penetration(sitehelper_project_find_slab_by_id(p,2),second,4)==SLAB_SUCCESS);
    assert(sitehelper_project_add_slab(p,1,outer,4,150,-25)); /* Zero penetrations. */
    ...
    assert(slab_add_penetration(sitehelper_project_find_slab_by_id(p,id),triangle,3)==SLAB_SUCCESS);
}
static void test_round_trip_and_copy(void)
{
    ...
    assert(clone.storeys[0].slabs.items[0].definition.penetrations.items!=p.storeys[0].slabs.items[0].definition.penetrations.items);
    ...
    assert(strstr(text,"sitehelper_project 12\n")==text);
...
static void write_input(int version, const char *penetrations)
{
    ...
    fprintf(f,"sitehelper_project %d\ndomain_id_next 3\nsettings 2400 90 35 600 1200 0 0 maximise\n"
        ...
        "vertex 0 0\nvertex 10000 0\nvertex 10000 8000\nvertex 0 8000\n",version);
    fputs(penetrations,f); fputs("end_slab\nend_storey\nend_project\n",f); assert(fclose(f)==0);
}
...
static void test_legacy_and_malformed(void)
{
    ...
    write_input(11,""); assert(sitehelper_project_load_file(&p,path)==SITEHELPER_PERSISTENCE_SUCCESS);
    assert(p.storeys[0].slabs.items[0].id==2 && p.domain_ids.next==3);
    assert(p.storeys[0].slabs.items[0].definition.penetrations.count==0);
    assert(!p.storeys[0].slabs.items[0].definition.penetrations.items);
    assert(slab_add_penetration(&p.storeys[0].slabs.items[0],hole,4)==SLAB_SUCCESS);
    test_assert_project_authoritative_equal(&before,&p);
    const char *bad[]={
        "", "penetrations -1\n", "penetrations nope\n", "penetrations 18446744073709551615\n",
        "penetrations 1\n", "penetrations 1\npenetration outline 2\n", "penetrations 1\npenetration outline -3\n",
        "penetrations 1\npenetration outline 18446744073709551615\n",
        "penetrations 1\npenetration outline 3\nvertex 1 nope\n",
        "penetrations 1\npenetration outline 3\nvertex 2147483648 1\n",
        "penetrations 1\npenetration outline 3\nvertex 1 1\nvertex 4 1\n",
        "penetrations 1\npenetration outline 3\nvertex 1 1\nvertex 4 1\nvertex 1 2\n", /* Missing end. */
        "penetrations 1\npenetration outline 4\nvertex 1 1\nvertex 4 4\nvertex 1 4\nvertex 4 1\nend_penetration\n",
        "penetrations 1\npenetration outline 4\nvertex -5 1\nvertex -1 1\nvertex -1 5\nvertex -5 5\nend_penetration\n",
        "penetrations 1\npenetration outline 4\nvertex 0 1\nvertex 4 1\nvertex 4 4\nvertex 0 4\nend_penetration\n",
        "penetrations 2\n" HOLE HOLE,
        "penetrations 2\n" HOLE "penetration outline 4\nvertex 3000 2500\nvertex 4500 2500\nvertex 4500 3500\nvertex 3000 3500\nend_penetration\n",
        "penetrations 1\n" HOLE "extra\n"
    };
    write_input(12,"penetrations 1\n" HOLE);
    assert(sitehelper_project_load_file(&p,path)==SITEHELPER_PERSISTENCE_SUCCESS);
    test_assert_project_authoritative_equal(&p,&before);
    for(size_t i=0;i<sizeof bad/sizeof *bad;i++) {
        write_input(12,bad[i]);
        unsigned char bytes[sizeof p]; memcpy(bytes,&p,sizeof p);
        SlabPenetration *items=p.storeys[0].slabs.items[0].definition.penetrations.items;
        PlanPosition *vertices=items[0].outline.vertices;
        assert(sitehelper_project_load_file(&p,path)!=SITEHELPER_PERSISTENCE_SUCCESS);
        assert(memcmp(bytes,&p,sizeof p)==0);
        assert(p.storeys[0].slabs.items[0].definition.penetrations.items==items && items[0].outline.vertices==vertices);
        test_assert_project_authoritative_equal(&before,&p);
    }
...
static void test_allocations(void)
{
    ...
    assert(load_failures>20 && load_failures<100 && copy_failures==5);
    printf("penetration project allocation sweep: %zu load, %zu restore failures; retries passed\n",load_failures,copy_failures);
...
int main(void)
{
    ...
    puts("project slab penetration tests passed"); return 0;
}
