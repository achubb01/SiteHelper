...
int main(void)
{
    ...
    DomainId first = sitehelper_project_add_room(&project);
    DomainId second = sitehelper_project_add_room(&project);
    ...
    assert(sitehelper_project_validate(&project).code == SITEHELPER_PROJECT_VALID);
    test_clone_project_authoritative(&project, &before);
    assert(wall_generate(&before.structure.walls[0], &before.settings));
    ...
    test_assert_framing_semantically_equal(&before.structure.walls[0].framing, &wall->framing);
