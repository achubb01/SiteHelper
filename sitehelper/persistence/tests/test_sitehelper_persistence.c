...
static void make_non_trivial_project(SiteHelperProject *project)
{
    ...
    for (int room_number = 0; room_number < 3; room_number++) {
        ...
        assert(wall_set_plan_specification(first_wall,(WallPlanSpecification){
            .thickness_mm=90 + room_number * 10,
            .alignment=room_number == 0 ? WALL_PLAN_ALIGNMENT_CENTER :
                (room_number == 1 ? WALL_PLAN_ALIGNMENT_LEFT_FACE :
...
static void test_version_ten_ordered_segments_round_trip(void)
{
    SiteHelperProject original;
    SiteHelperProject loaded;
    sitehelper_project_init(&original);
    ...
    assert(strstr(text, "segment 1000 2000 4600 6800 plan_spec 90 center openings 1") != NULL);
    assert(strstr(text, "segment 4600 6800 1000 2000 plan_spec 90 center openings 1") != NULL);
...
int main(void)
{
    ...
    test_version_ten_ordered_segments_round_trip();
