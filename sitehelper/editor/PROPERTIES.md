...
2. **Selection representation.** `EditorSelectionKind` distinguishes `NONE`, `WALL`, `OPENING` and `WALL_MEMBER`. Priority 15 adds a separate `EditorSelectionScope` (`NONE`, `PLAN`, `WALL_ELEVATION`): kind identifies the object, scope identifies its context. The struct stores scope, `wall_id`, `opening_id` (only meaningful for OPENING), and the existing by-value `WallSelection`. Setters require an explicit valid non-NONE scope and clear the complete selection on invalid input. Clearing resets kind, scope and payload. It contains no persistent collection pointers. Setters do not alter navigation. Plan Select sets navigation and Plan-scoped Wall selection as separate operations, retaining the existing nearest-wall/tie policy. Elevation member and Opening hits explicitly acquire Elevation scope. See [selection context](SELECTION.md).
...
3. **Inspector projection.** `sitehelper_editor_inspect_properties(editor, project, output)` in `editor_properties.h` returns a caller-owned `EditorProperties` tagged value. `EditorWallProperties` contains `wall_id`, ordered `segment`, derived `length_mm`, `resolved_stud_height` and `resolved_stud_spacing`. `EditorOpeningProperties` contains owner `wall_id` and a complete by-value `Opening definition`, including its ID, type, U, Z, dimensions, custom flag and stored allowances. Header comments identify editable and read-only fields.
   ...
   Resolution first checks that selection scope matches the active view, then resolves IDs within `current_storey_id`, independent of `current_wall_id`. Every request reads current project definitions/settings; editor state stores no property cache. NONE, member selection, mismatched scope, missing IDs and invalid context return zero with a cleared snapshot, even before reconciliation. Kind and scope remain orthogonal; inspection does not hard-code Wall properties to Plan or Opening properties to Elevation. An opening snapshot reports stored allowance values even when custom allowances are disabled; hit testing and generation use effective allowances instead.
